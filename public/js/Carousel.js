...
(function ($) {
    ...
    function scroll($containerCarousel, direction) {
        if ($containerCarousel.data('isanimating')) {
            return;
        }
        var config = $containerCarousel.data('config');
        var halfShowNum = (config.num - 1) / 2;
        var scales, i, newIndex;
        var totalNum = $containerCarousel.data('totalNum');
        var targetCss;
        var firstIndexBeforeScroll, lastIndexBeforeScroll;
        if (direction === 'left') {
            newIndex = ($containerCarousel.data('index') - 1 + totalNum) % totalNum;
        } else if (direction === 'right') {
            newIndex = ($containerCarousel.data('index') + 1) % $containerCarousel.data('totalNum');
        } else {
            return;
        }
        // $container.find('ul li').stop(true, true);
        var tempIndexsInfo = getShowIndexs($containerCarousel);
        firstIndexBeforeScroll = tempIndexsInfo.indexs[0];
        lastIndexBeforeScroll = tempIndexsInfo.indexs[config.num - 1];
        $containerCarousel.data('index', newIndex);
        var showIndexsInfo = getShowIndexs($containerCarousel);
         ...
         */
        $containerCarousel.find('ul li').each(function (index, element) {

            i = showIndexsInfo.hashIndexs[index];

            if (i !== undefined) {
                scales = Math.pow(config.scale, Math.abs(i - halfShowNum));
                $containerCarousel.data('isanimating', true);
                $(element).css({
                    display: 'block',
                    ...
                    $containerCarousel.data('isanimating', false);
                });
    ...
    function getShowIndexs($containerCarousel) {
        var showIndexs = [];
        var temp;
        var halfShowNum = ($containerCarousel.data('config').num - 1) / 2;
        var currentIndex = $containerCarousel.data('index') || 0;
        var totalNum = $containerCarousel.data('totalNum') || 0;
    ...
    function initStyle($containerCarousel) {
        var showIndexsInfo = getShowIndexs($containerCarousel);
        ...
        var containerWidth = $containerCarousel.width();
        var containerHeight = $containerCarousel.height();
        if (containerWidth < listWidth) {
            $containerCarousel.width(listWidth);
        }
        if (containerHeight < config.maxHeight) {
            $containerCarousel.height(config.maxHeight);
        }
        $containerCarousel.find('ul li img').css({
            width: "100%",
            height: "100%"
        });
        $containerCarousel.find('ul').css({
            position: 'relative',
            width: listWidth,
            ...
            marginLeft: '50%',
            left: -listWidth / 2,
            top: ($containerCarousel.height() - config.maxHeight) / 2
        });

        $containerCarousel.find('.left').css({
            position: 'absolute',
            left: -60,
            top: '40%',
            'z-index': 9999 + $containerCarousel.data('totalNum') + 1
        });

        $containerCarousel.find('.right').css({
            position: 'absolute',
            right: -60,
            top: '40%',
            'z-index': 9999 + $containerCarousel.data('totalNum') + 1
